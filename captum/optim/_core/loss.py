...
def module_op(
    ...
    if other is None and math_op == operator.neg:
        ...
        target = (
            self.target if hasattr(self.target, "__iter__") and not isinstance(self.target, torch.jit.ScriptModule) else [self.target]
        ) + (other.target if hasattr(other.target, "__iter__") and not isinstance(self.target, torch.jit.ScriptModule) else [other.target])
    else:
        raise TypeError(
            "Can only apply math operations with int, float or Loss. Received type "
            + str(type(other))
